  ...
  return (
    <div className="min-h-screen bg-rose-300/30 text-gray-800 p-6">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        {/* SEARCH BAR */}
        <div className="bg-white/30 p-4 rounded-2xl shadow-2xl shadow-purple-700/30 hover:border hover:border-purple-300 flex items-center gap-3">
        ...
        {filteredCategories.map((category) => (
          <div key={category.key} className="space-y-3">
            ...
            <h2 className="text-xl font-semibold text-purple-800 flex items-center gap-2">
              <Folder size={20} className="text-purple-500" /> {category.name}
            </h2>
            ...
            <div className="space-y-4">
              {category.forms.map((form, i) => (
                <div
                  key={i}
                  className="bg-white/30 border border-purple-300 rounded-2xl p-5 shadow-purple-500 hover:shadow-[0_0_35px_rgba(118,0,255,0.25)] transition-all"
                >
                  <div className="flex items-center justify-between">
                    ...
                    <p className="text-sm px-3 py-1 bg-purple-400/30 border border-purple-500 rounded-full text-black">
                      {form.fee}
                    </p>
                  </div>
                  ...
                  <button className="mt-4 bg-purple-500 text-white  px-5 py-2 rounded-xl hover:bg-indigo-600 transition">
                    Apply Now
                  </button>
                </div>
              ))}
        ...
        {filteredCategories.length === 0 && (
          <p className="text-center text-gray-700 mt-10">
            No forms found matching "<span className="text-purple-900">{query}</span>"
          </p>
        )}
      </div>
    </div>
