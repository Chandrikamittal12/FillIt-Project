...
const Communication = () => {
  ...
  return (
    <div className="min-h-screen p-6 bg-white/30 text-purple-800">
      <h2 className="text-2xl font-semibold mb-6">Communication</h2>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        
        {/* ---------------- LEFT SIDEBAR (User List) ---------------- */}
        <div className="bg-white/30 p-4 rounded-2xl border border-purple-300/30 shadow-2xl shadow-purple-600/30 h-[75vh] flex flex-col">
          
          {/* Search */}
          <div className="bg-white/30 border border-purple-500/30 px-3 py-2 rounded-xl flex items-center gap-2 mb-4">
            ...
            {filteredUsers.map((u) => (
              <div
                ...
                className={`p-3 rounded-xl cursor-pointer transition-all 
                  ${selectedChat === u.id ? "bg-purple-700/30" : "bg-rose-300/30 hover:bg-purple-300/30"}
                `}
              >
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-purple-600/30 text-black rounded-full">
                    <User size={18} />
                  </div>
                  <div>
        ...
        <div className="lg:col-span-2 bg-white/30 p-4 rounded-2xl border border-purple-300/30 shadow-2xl shadow-purple-500/30 h-[75vh] flex flex-col">

          {/* No Chat Selected */}
          {!selectedChat && (
            <div className="h-full flex gap-2 items-center justify-center text-gray-800">
              <MessageCircle size={40} className="mb-2 text-indigo-500" />
                  ...
                  >
                    <div
                      className={`max-w-xs md:max-w-sm lg:max-w-md p-3 rounded-xl text-sm
                        ${
                          msg.sender === "admin"
                            ? "bg-purple-800 text-white shadow-[0_0_15px_rgba(128,0,255,0.35)]"
                            : "bg-white text-gray-800"
                        }
                      `}
                    >
                      <p>{msg.message}</p>
                      <p className="text-[10px] mt-1 text-gray-300/90 flex justify-between">
                        <span>{msg.time}</span>
                        {msg.status && (
